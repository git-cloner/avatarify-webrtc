...
def load_detector() :
    ...
    if predictor_face == None:
        predictor_face = dlib.shape_predictor(r'./model/shape_predictor_68_face_landmarks.dat')
...
def face_avatar(frame,im1,landmarks1,im1_mask):
    ...
    landmarks2 = get_face_landmarks(img_gray, detector_face, predictor_face)  # 68_face_landmarks
...
class VideoTransformTrack(MediaStreamTrack):
    ...
    async def recv(self):
        ...
        if self.skip_frame == 0:
            ...
            img = face_avatar(img,self.im1,self.landmarks1,self.im1_mask)
            if img is not None:
